...
def main():
    ...
    parser.add_argument("--test_file", default="test", type=str,
                        help="Name of the test file without file suffix (only in conjunction with do_infer=True)")
                ...
                for line in lines:
                    ...
                    for mention in doc['candidates']:
                        if mention['offset'][1]>len(predictions[Cnt+mention['sent_id']]):
                            logger.warning("Candidate mention sent idx not in doc?",len(doc['content'][mention['sent_id']]['tokens']),len(predictions[Cnt+mention['sent_id']]))
